...
- 경험은 타임라인, 기술은 입체적인 세 개의 패널, 프로젝트는 개별 조명이 있는 쇼케이스로 구성합니다. 제목 크기는 최대 44px, 첫 화면 이름은 최대 80px입니다.
- 상단바 없이 첫 화면 링크와 하단의 맨 위로 이동 링크로 페이지를 탐색합니다.
- 마우스가 있는 환경에서는 패널 조명과 프로젝트 기울기가 포인터에 반응합니다. 입력이 없을 때는 반복 애니메이션을 실행하지 않습니다.
